...
import com.em.sdk.utils.exceptions.MessageDecodingException;
import com.em.sdk.utils.exceptions.MessageEncodingException;
...
public final class Numeric {
    ...
    public static String cleanHexOrAoaPrefix(String input) {
        if (containsEMPrefix(input) && input.length() >= 42) {
            return input.substring(2);
        } else if (containsHexPrefix(input)) {
            return input.substring(2);
        } else {
            return input;
        }
    ...
    public static String prependAOAPrefix(String input) {
        if (!containsEMPrefix(input)) {
            return EM_PREFIX + input;
        } else {
            return input;
        }
    }
    ...
    public static boolean containsEMPrefix(String input) {
        return input.length() > 1 && (input.charAt(0) == 'e' || input.charAt(0) == 'E') &&
               (input.charAt(1) == 'm' || input.charAt(1) == 'M');
    }
    ...
    public static BigInteger toBigInt(String hexValue) {
        String cleanValue = cleanHexOrAoaPrefix(hexValue);
    ...
    public static byte[] hexStringToByteArray(String input) {
        String cleanInput = cleanHexOrAoaPrefix(input);
