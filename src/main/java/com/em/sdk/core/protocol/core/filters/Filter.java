...
public abstract class Filter<T> {
    ...
    public void run(ScheduledExecutorService scheduledExecutorService, long blockTime) {
        try {
            EMFilter ethFilter = sendRequest();
    ...
    private void getInitialFilterLogs() {
        try {
            Optional<Request<?, EMLog>> maybeRequest = this.getFilterLogs(this.filterId);
            EMLog ethLog = null;
    ...
    private void pollFilter(EMFilter ethFilter) {
        EMLog ethLog = null;
        try {
            ethLog = web3j.emGetFilterChanges(filterId).send();
        } catch (IOException e) {
            throwException(e);
        }
        if (ethLog.hasError()) {
    ...
    abstract EMFilter sendRequest() throws IOException;

    abstract void process(List<EMLog.LogResult> logResults);

    public void cancel() {
        schedule.cancel(false);

        try {
            EMUninstallFilter ethUninstallFilter = web3j.emUninstallFilter(filterId).send();
     ...
     */
    protected abstract Optional<Request<?, EMLog>> getFilterLogs(BigInteger filterId);
